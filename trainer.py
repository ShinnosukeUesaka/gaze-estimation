...
class Trainer(object):
    def __init__(self, config, data_loader):
        ...
        self.log_dir = os.path.join(config.ckpt_dir, 'log') 
        if os.path.exists(self.log_dir) and os.path.isdir(self.log_dir) and config.is_train:
    ...
    def train(self):
        ...
        if self.load_ckpt is not None:
            print('load previous ckpt: ', self.load_ckpt)
            self.load_checkpoint(is_strict=False, input_file_path=self.load_ckpt)
        ...
        self.writer.close()
        # np.savetxt(os.path.join(self.ckpt_dir,'train_log.txt') ,np.array(train_txtlog))
    ...
    def test(self):
        ...
        for i, (input_img) in enumerate(self.test_loader):
            ...
            if i % self.print_freq == 0 and i is not 0:
                print('--------------------------------------------------------------------')
                msg = "test error: {:.3f}  -  loss_gaze: {:.5f}"
                print(msg.format(errors.avg, losses_gaze.avg))
                print('For XGaze-test, there are no labels for computing test error, so here is 0')

            #     errors.reset()
            #     losses_gaze.reset()
        if save_index != self.num_test:
            print('the test samples save_index ', save_index, ' is not equal to the whole test set ', self.num_test)
        np.savetxt(os.path.join(self.ckpt_dir,"pred_gaze.txt"), pred_gaze_all, delimiter=',')
        print('Final error and loss --------------------------------------------------------------------')
        msg = "test error: {:.3f}  -  loss_gaze: {:.5f}"
        print(msg.format(errors.avg, losses_gaze.avg))
        print('Tested on : ', pred_gaze_all.shape[0], ' samples')
        return msg.format(errors.avg, losses_gaze.avg)

    def test_wlabel(self):
        """
        Test the pre-treained model on the whole test set. Note there is no label released to public, you can
        only save the predicted results. You then need to submit the test resutls to our evaluation website to
        get the final gaze estimation error.
    ...
    def load_checkpoint(self, input_file_path='./ckpt/ckpt.pth.tar', is_strict=True):
        ...
        print('ckpt opotim state: ')
        print(list(ckpt['optim_state'].keys()))
